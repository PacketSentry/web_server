...
#
# ... (Ensure these Failsafe Routes are present) ...

@app.route('/dashboard')
@login_required
def dashboard():
    # FAILSAFE: Redirect HTMX requests to the update route
...
def dashboard_update():
    data = get_dashboard_data()
    response = make_response(render_template('dashboard_content.html', **data))
...
def api_sync():
    ...
    for log in data.get('logs', []):
        ...
        db.session.add(LogEntry(user_id=api_user.id, timestamp=datetime.datetime.fromtimestamp(log[0]), app_name=log[1], download_speed=log[2], upload_speed=log[3], src_ip=enc_src, dst_ip=enc_dst))
